...
unsigned int nStakeTargetSpacing = 51;			// 51 sec block spacing
...
const string strMessageMagic = "PopularCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight == 1)
    {
        nSubsidy = 35000000 * COIN;  // 35 Million coins first block
        return nSubsidy + nFees;
    }
    
  if (nHeight >= 2 && nHeight < 12000)
    {
		nSubsidy = 1 * COIN;
		...
		// Every 2 hours or 12 times a day
			nSubsidy *= 25;
		} else if (mod_floor(nHeight, 71) == 0) {
		// 1 time per hour 
			nSubsidy *= 10;
		} else if (mod_floor(nHeight, 18) == 0) {
		// 4 times per hour 
			nSubsidy *= 5;
		} else if (mod_floor(nHeight, 9) == 0) {
		// 10 times per hour equal to 9 blocks
			nSubsidy *= 2;
		}
  ...
  if (nHeight >= 12000 && nHeight < 51000)
    {
		nSubsidy = 10 * COIN;
  ...
  if (nHeight >= 51000 && nHeight < 121000)
    {
		nSubsidy = 4 * COIN;
  ...
  if (nHeight >= 121000 && nHeight < 181000)
    {
		nSubsidy = 2 * COIN;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in popcoin
    bool fStrictPayToScriptHash = true; // Always active in popcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "popcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "January 28, 2014, PopularCoin Comes To Existence - V2 Gen Block April 2017 1490838410";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x0707cdfa92c366f4d2734b0b2edc647dc1002e8fbc2282ff58a2f8ec3b2a202e"));
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // popcoin: safe mode for high alert
            }
        }
    }
